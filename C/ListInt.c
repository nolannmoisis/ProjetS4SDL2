...
ListInt* ListInt_Reverse(ListInt* list){
    ListInt* listReverse = ListInt_create();
    AssertNew(listReverse);

    ListIntIter* iterList = ListIntIter_create(list);
    AssertNew(iterList);

    while (ListIntIter_isValid(iterList)){
        int tmp = ListIntIter_get(iterList);
        ListInt_insertFirst(listReverse, tmp);
